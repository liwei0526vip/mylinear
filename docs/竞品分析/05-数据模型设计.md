...
| id | UUID | PK | 主键 |
| workspace_id | UUID | FK → Workspace | 所属工作区 |
| email | VARCHAR(255) | NOT NULL, UNIQUE | 登录邮箱（支持修改，需验证） |
| name | VARCHAR(255) | NOT NULL | 全名（Full name），用于正式展示 |
| username | VARCHAR(50) | NOT NULL, UNIQUE | 用户名（Username），用于 @mention |
| avatar_url | TEXT | | 头像 URL（Profile picture） |
...
**索引**：
- `UNIQUE (workspace_id, username)` — 工作区内用户名唯一
- `UNIQUE (email)` — 全局邮箱唯一
