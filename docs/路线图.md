...
| 87 | 用户个人资料（Profile） | ✅ | 头像、邮箱、全名、用户名管理 |
| 88 | Workspace 管理 | ✅ | 名称/基本配置 |
| 89 | Teams 管理 | ✅ | 创建/成员/权限 |
| 90 | 团队标识符 | ✅ | 用于生成 Issue ID（如 ENG-123） |
| 91 | 角色权限（Admin/Member） | ✅ | 基本角色体系 |
| 92 | 设置复用 | 🔵 | 从现有团队复制 workflow 和 cycle 配置 |
| 93 | 时区设置 | 🔵 | 支持 Cycle 时间计算 |
| 94 | Team Owner 角色 | 🔵 | 团队级管理权限 |
| 95 | 私有团队 | 🔴 | 权限隔离 |
| 96 | Guest 角色 | 🔴 | 受限访问 |
| 97 | 导入/导出 | 🟡 | Jira/CSV |
| 98 | REST API | 🟡 | 完整 CRUD |
...
| 100 | Webhooks | 🟡 | 事件推送 |
| 101 | 审计日志 | 🟠 | 操作记录 |
| 102 | SSO/LDAP | 🟠 | SAML/SCIM 企业认证 |
| 103 | Workspace Owner 角色 | 🟠 | 工作区最高权限 |
| 104 | 桌面客户端 (Tauri) | 🔴 | 原生桌面应用 |
...
- [ ] 用户个人资料（Profile）：头像上传、邮箱管理、全名/用户名设置
- [ ] 角色权限：Admin / Member
- [ ] Workspace：CRUD、基本设置
...
- [ ] 16 张核心表设计（UUID 主键）
- [ ] User 表增加 `username` 字段（用于 @mention）
...
| 用户个人资料（Profile） | ★★★★☆ | 头像、全名、用户名（@mention）、邮箱管理，用户身份的基础 |
...
| User 表 `username` 字段 | 支持 @mention 引用，与 `name`（全名）分离，匹配 Linear Profile 设计 |
